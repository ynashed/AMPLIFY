...
r"""
Conversion script to convert pytorch/xformers AMPLIFY checkpoint into nemo checkpoint.
  Example to run this conversion script:
    python convert_checkoint_to_nemo.py \
     --input_name_or_path <path_to_checkpoint_file.pt> \
     --output_path <path_to_output_nemo_file.nemo>
     --precision bf16 \
     --llama31 True 
"""
...
def get_args():
    ...
    parser.add_argument(
        ...
        default=None,
        required=True,
        help="Path to trained AMPLIFY checkpoint file. Ex: /data/models/AMPLIFY/AMPLIFY_350M/pytorch_model.pt",
    )
    parser.add_argument(
        "--config_name_or_path",
        type=str,
        default=None,
        required=True,
        help="Path to YAML file for AMPLIFY config. Ex: /data/models/AMPLIFY/AMPLIFY_350M/config.yaml",
    )
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            os.path.dirname(__file__), '../../examples/nlp/language_modeling/conf/megatron_amplify_config.yaml'
        ),
        required=False,
...
def load_config(args, amplify_config):
    nemo_config = OmegaConf.load(args.hparams_file).model

    if amplify_config['rope_theta'] is not None:
        nemo_config['rotary_base'] = amplify_config['rope_theta']
    nemo_config.encoder_seq_length = amplify_config['max_length']
    nemo_config.num_layers = int(amplify_config['num_hidden_layers'])
    nemo_config.hidden_size = amplify_config['hidden_size']
    nemo_config.ffn_hidden_size = amplify_config['intermediate_size']
    nemo_config.num_attention_heads = amplify_config['num_attention_heads']
    ...
    if 'vocab_path' in amplify_config:
        nemo_config.tokenizer.model = amplify_config['vocab_path']
    else:
    ...
    base = 128
    while amplify_config['vocab_size'] % base != 0:
        base //= 2
...
def convert(args):
    ...
    model, tokenizer = amplify.AMPLIFY.load(args.input_name_or_path, args.config_name_or_path)
    amplify_config = vars(model.config)
    print(f"amplify_config: {amplify_config}")
    ...
    hidden_size = amplify_config["hidden_size"]
    head_num = amplify_config["num_attention_heads"]
    head_size = hidden_size // head_num
    num_layers = amplify_config["num_hidden_layers"]
    ...
    if f'transformer_encoder.0.rotary_emb.inv_freq' in model.state_dict():
        rotary_embed_weight = model.state_dict()[f'transformer_encoder.0.rotary_emb.inv_freq']
        if mcore_gpt:
    ...
    for l in range(int(num_layers)):
        ...
        old_tensor_shape = model.state_dict()[f'transformer_encoder.{l}.q.weight'].size()
        new_q_tensor_shape = (head_num, head_size) + old_tensor_shape[1:]
        new_kv_tensor_shape = (num_query_groups, head_size) + old_tensor_shape[1:]
        q = model.state_dict()[f'transformer_encoder.{l}.q.weight'].view(*new_q_tensor_shape)
        k = model.state_dict()[f'transformer_encoder.{l}.k.weight'].view(*new_kv_tensor_shape)
        v = model.state_dict()[f'transformer_encoder.{l}.v.weight'].view(*new_kv_tensor_shape)
        ...
        # attention dense
        o_weight = model.state_dict()[f'transformer_encoder.{l}.wo.weight']
        if mcore_gpt:
        ...
        # MLP
        mlp_down_weight = model.state_dict()[f'transformer_encoder.{l}.mlp.gate_proj.weight']
        mlp_gate_weight = model.state_dict()[f'transformer_encoder.{l}.mlp.up_proj.weight']
        if mcore_gpt:
        ...
        mlp_up_weight = model.state_dict()[f'transformer_encoder.{l}.mlp.down_proj.weight']
        if mcore_gpt:
        ...
        # LayerNorm
        input_ln_weight = model.state_dict()[f'transformer_encoder.{l}.input_layernorm.weight']
        if mcore_gpt:
        ...
        post_attn_ln_weight = model.state_dict()[f'transformer_encoder.{l}.post_attention_layernorm.weight']
        if mcore_gpt:
    ...
    if 'vocab_path' not in amplify_config:
        if args.llama31:
            if amplify_config['num_hidden_layers'] == 32:
                model.cfg.tokenizer.update(type='meta-llama/Meta-Llama-3.1-8B')
            elif amplify_config['num_hidden_layers'] == 80:
                model.cfg.tokenizer.update(type='meta-llama/Meta-Llama-3.1-70B')
            elif amplify_config['num_hidden_layers'] == 126:
